...
bool NifMeshExporterFallout4::CreateComplexFace(MStatus& stat, MObject object, vector<Color4> niColors, MItMeshPolygon& itPoly, ComplexFace& cf)
{
...
void NifMeshExporterFallout4::ExportMesh(MObject dagNode)
{
	...
	MIntArray face_material_indices;
	...
	stat = visibleMeshFn.getConnectedShaders(0, shaders, face_material_indices);
	...
	for (unsigned int shader_num = 0; shader_num < shaders.length(); ++shader_num)
	{
		...
		for (size_t f = 0; f < face_material_indices.length(); ++f)
		{
			if (face_material_indices[f] == shader_num)
			{
				shader_is_used = true;
				break;
			}
	...
	map<int, int> original_face_indices;
	...
	if (segment_nodes.length() == 0)
	{
		...
		for (; !itPoly.isDone(); itPoly.next())
		{
			...
			original_face_indices[nif_faces.size() - 1] = itPoly.index();
		}
	}
	else
	{
		...
		for (int segment_node_index = 0; segment_node_index < segment_nodes.length(); segment_node_index++)
		{
			...
			if (sub_segments_plug.numConnectedElements() > 1)
			{
				for (int sub_segment_plug_index = 0; sub_segment_plug_index < sub_segments_plug.numConnectedElements(); sub_segment_plug_index++)
				{
					MPlug sub_segment_connection = sub_segments_plug.connectionByPhysicalIndex(sub_segment_plug_index);
					MPlugArray sub_segments_connections;

					sub_segment_connection.connectedTo(sub_segments_connections, true, false);

					if (sub_segments_connections.length() > 0)
					{
						...
						target_face_plug.connectedTo(blind_data_connections, true, false);
						...
						if (blind_data_connections.length() > 0)
						{
							...
							for (; !it_polygons.isDone(); it_polygons.next())
							{
								if (it_polygons.polygonVertexCount() >= 3)
								{
										...
										meshFn.hasBlindDataComponentId(it_polygons.index(), MFn::Type::kMeshPolygonComponent, blind_data_id))
									{
										if (processed_faces_indices.find(it_polygons.index()) == processed_faces_indices.end())
										{
											...
											original_face_indices[nif_faces.size() - 1] = it_polygons.index();
										}
									}
								}
							}
			...
			else if (sub_segments_plug.numConnectedElements() == 1)
			{
				MPlug sub_segment_connection = sub_segments_plug.connectionByPhysicalIndex(0);
				MPlugArray sub_segments_connections;
	...
	bs_complex_shape.SetSegments(segments);

	//Set shader/face association
	if (nif_faces.size() != face_material_indices.length())
	{
	...
	{
		nif_faces[face_index].propGroupIndex = face_material_indices[original_face_indices[face_index]];
	}
